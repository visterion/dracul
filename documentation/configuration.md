...
| `DRACUL_EXECUTOR_MAX_POSITIONS` | `dracul.executor.max-positions` | `8` | `VetoService` rejects (`MAX_POSITIONS`) a new entry once `executor_position` has this many `OPEN` rows. Raised from `5` to `8` in SP2 (2026-09). |
...
| `DRACUL_EXECUTOR_RISK_PCT` | `dracul.executor.risk-pct` | `0.01` | Fraction of `total-budget` a single tranche may risk in account currency. `PositionSizer` computes `qtyRisk = floor(riskBudget / (rPerShare × fx))` from the LOGICAL stop and sizes `qty = min(qtyNotional, qtyRisk)`; `order_json.sizing_basis` names which cap bound. A stop distance that exceeds the whole budget even for one share rejects terminally as `RISK_TOO_WIDE`, so high-priced, high-ATR names leave the tradeable universe at the default budget — visible in `executor_decision.reject_reason`. `position_risk` (logical) stays the figure the HEAT_LIMIT veto consumes; `position_risk_broker` (from the resting leg) is logged only. |
| `DRACUL_EXECUTOR_RULE_VERSION` | `dracul.executor.rule-version` | `exec-v0.6` | The active rule-version tag (`RuleVersionProvider`) stamped onto every `decision_log` row, so a later change in prompt/thresholds is traceable in the audit trail. `RuleVersionProvider.seed()` is insert-if-absent, so bumping this default automatically seeds a new `rule_versions` row on next boot — no manual migration needed. `exec-v0.6` (SP2, 2026-09) lowers `min-confidence` to `0.40`, raises `max-positions` to `8`, and adds the `MECHANISM_BUDGET` veto; `exec-v0.5` adds the buffered/capped broker stop, `atr_short`/`atrEff`, risk-based sizing with `RISK_TOO_WIDE`, and drops `NEW_HIGH` from tranche-2 eligibility in favor of gating on `entry_filled_at`; `exec-v0.4` is the sim-completion stamp (hybrid kill hard trigger, scale-out ladder, entry GTD) and also fixed a `confidence_min` drift in the seeded `rule_versions.params` audit blob (`0.6` → `0.65`, matching the `min-confidence` runtime value at the time, which was already `0.65`); `exec-v0.3` stays in the table as history — see `documentation/operations.md` for the versioning-change discipline. SP2 (2026-09): confidence floor 0.40, MECHANISM_BUDGET, max-positions 8. |
