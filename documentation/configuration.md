...
| `DRACUL_DEPOTS_EQUITY_SNAPSHOT_ENABLED` | `true` | Kill-Switch für den Equity-Snapshot-Job. |
| `DRACUL_DEPOTS_EQUITY_SNAPSHOT_DAILY_CRON` | `0 45 21 * * 1-5` | Tagesschluss-Snapshot, UTC. 105 min nach EDT-Schluss, 45 min nach EST-Schluss. |
| `DRACUL_DEPOTS_EQUITY_SNAPSHOT_INTRADAY_CRON` | `0 5,20,35,50 13-21 * * 1-5` | Intraday-Snapshot für den `1T`-Chart, UTC. |
...
|---|---|---|
| `DRACUL_LOG_DIR` | `logs` | Directory for the rotating log files. In production it is mounted onto a host directory, so the logs survive the container being recreated — see [operations.md](./operations.md#log-retention-14-days). |
...
| `DRACUL_EXECUTOR_AGORA_TIMEOUT_MS` | `dracul.executor.agora-timeout-ms` | `8000` | Timeout (ms) for calls to Agora's trading tools — **connect and read, reads and writes alike**. It governs every tool: the reads (`get_account`, `get_positions`, `get_orders`, `get_closed_positions`, `get_order_by_ref`) end-to-end, and the four write tools on the connect leg only. Timeouts surface as `BROKER_ERROR`. |
| `DRACUL_EXECUTOR_AGORA_WRITE_TIMEOUT_MS` | `dracul.executor.agora-write-timeout-ms` | `30000` | **Read timeout only**, and only for the four broker-write tools (`place_bracket`, `flatten`, `modify_bracket`, `cancel_order`); their connect timeout stays on `agora-timeout-ms`. Agora paces Saxo order writes ~1.1 s apart and may add one clamped 429 block, so one write tool call can cover several spaced requests. **Invariant:** `(N_paced − 1) × min-interval + N_paced × request time + max-block < agora-write-timeout-ms`, with `N_paced = 4L + 1` and `L` = protective legs resting on the symbol — `L = 2` (today's two-tranche position): 9 writes, 13.6 s; `L = 4` (both tranches carrying a take-profit): 17 writes, 24.0 s; both under 30 s at a 1100 ms interval. This is a **cut-off, not a bound**: Agora applies its own 10 s per request, so a badly degraded broker can exceed the cut-off and Dracul then books a non-transient `BROKER_ERROR` while Agora finishes the sequence; the next reconcile pass re-reads the broker. |
...
| `DRACUL_EXECUTOR_MIN_CONFIDENCE` | `dracul.executor.min-confidence` | `0.40` | `VetoService` rejects (`LOW_CONFIDENCE`) any signal whose `confidence` is below this threshold. Raised from `0.6` to `0.65` as part of the entry-completeness work, then lowered to `0.40` in SP2 (2026-09) once echo counterfactual analysis showed the higher floor was cutting profitable lower-confidence signals; `application.yaml`'s `${DRACUL_EXECUTOR_MIN_CONFIDENCE:0.40}` fallback and the `@Value` default in `ExecutorWebhookController` are now aligned at `0.40`, so the value actually seen at runtime absent an explicit env var is `0.40`. Set the env var to override. |
| `DRACUL_EXECUTOR_MAX_POSITIONS` | `dracul.executor.max-positions` | `25` | `VetoService` rejects (`MAX_POSITIONS`) a new entry once `executor_position` has this many `OPEN` rows. Raised from `5` to `8` in SP2 (2026-09), then to `25` in the exec-v0.7 paper capital scale-up (2026-09). |
| `DRACUL_EXECUTOR_MECHANISM_BUDGET_PCT` | `dracul.executor.mechanism-budget-pct` | `MERGER_ARB:0.20,QUALITY_52W_LOW:0.15` | Per-mechanism share of `total-budget` that new entries may occupy; `VetoService` rejects (`MECHANISM_BUDGET`, transient) when open exposure in the signal's mechanism plus one tranche would exceed it. Entry cap only — `add_tranche` is not gated. Malformed spec fails startup. |
...
| `DRACUL_EXECUTOR_ATR_SHORT_PERIOD` | `dracul.executor.atr-short-period` | `5` | Look-back period of the SECOND ATR requested in the same `get_indicators` call, under the explicit label `atr_short`. ATR22 is computed before the report day for a same-day PEAD entry and understates post-report volatility, so `atrEff = max(atr22, atr_short)` is what the stop window, the stop clamp, the sizing call, the broker-stop buffer and the chandelier all use. The short window can only WIDEN a stop, never tighten one. Missing (too few bars, or a label collision at Agora) is fail-soft: `atrEff` falls back to ATR22 and one `maintenance indicators unavailable: atr_short …` line is logged per symbol per run. The soft-trigger chandelier and the LLM's `atr` field deliberately stay on ATR22 — widening the soft trigger would silence it. |
| `DRACUL_EXECUTOR_BROKER_STOP_BUFFER_ATR` | `dracul.executor.broker-stop-buffer-atr` | `1.0` | How many `atrEff` the protective leg resting at the broker sits AWAY from the logical `active_stop` (below it for a long, above for a short). The close-based hard trigger keeps deciding on `active_stop`; the broker leg becomes a catastrophe backstop, so an intraday wick can no longer close a position the close-based rule would have kept. `0` is the exact identity — the bracket then receives the logical stop verbatim and the ratchet the raw chandelier — which is how the feature is proved additive. The buffered price never crosses the logical stop, never goes non-positive (it clamps to `logical × 0.5` and records `broker_stop_clamped`), and on the ratchet never moves against the position (it records `broker_stop_lags` when the monotonic floor binds). Consequence, stated plainly: an ordinary breach now exits by MARKET order at the next open instead of an intraday stop fill. |
| `DRACUL_EXECUTOR_MAX_BROKER_STOP_PCT` | `dracul.executor.max-broker-stop-pct` | `0.20` | Proximity cap on entry and add-tranche only: the total distance `entry − broker_stop` may not exceed this fraction of the entry price. Saxo rejects a bracket whose legs sit outside a proximity band (`TooFarFromEntryOrder`), which takes the protective stop down with it. The buffer shrinks first; if the LOGICAL stop alone already exceeds the cap the broker stop equals it and today's behaviour applies — beyond the band Saxo rejects the bracket with `TooFarFromEntryOrder`; Agora reports it with the leg named and nothing is placed; Dracul books a transient `BROKER_ERROR` and the signal is retired after `max-broker-attempts`. Recorded as `broker_stop_capped` in `order_json`. There is no cap on the ratchet path: the band is a statement about the distance from an entry order, and a ratchet has none. |
...
| `DRACUL_EXECUTOR_RISK_PCT` | `dracul.executor.risk-pct` | `0.005` | Fraction of `total-budget` a single tranche may risk in account currency. `PositionSizer` computes `qtyRisk = floor(riskBudget / (rPerShare × fx))` from the LOGICAL stop and sizes `qty = min(qtyNotional, qtyRisk)`; `order_json.sizing_basis` names which cap bound. A stop distance that exceeds the whole budget even for one share rejects terminally as `RISK_TOO_WIDE`, so high-priced, high-ATR names leave the tradeable universe at the default budget — visible in `executor_decision.reject_reason`. `position_risk` (logical) stays the figure the HEAT_LIMIT veto consumes; `position_risk_broker` (from the resting leg) is logged only. Lowered from `0.01` to `0.005` in the exec-v0.7 paper capital scale-up (2026-09). |
| `DRACUL_EXECUTOR_RULE_VERSION` | `dracul.executor.rule-version` | `exec-v0.8` | The active rule-version tag (`RuleVersionProvider`) stamped onto every `decision_log` row, so a later change in prompt/thresholds is traceable in the audit trail. `RuleVersionProvider.seed()` is insert-if-absent, so bumping this default automatically seeds a new `rule_versions` row on next boot — no manual migration needed. `exec-v0.8` (2026-09) shortens `cooldown-days` from `10` to `3`: prod counterfactual analysis of COOLDOWN-vetoed signals showed they averaged +1.17 R after 20 days (16/16 positive, n=16), i.e. the veto was filtering out winners; all other gates are unchanged from exec-v0.7. `exec-v0.7` (2026-09) is the paper capital scale-up for learning throughput: `total-budget` `10000` → `100000`, `tranche-count` `10` → `25` (tranche `1000` → `4000`), `risk-pct` `0.01` → `0.005`, `heat-pct` `0.06` → `0.15` (heat limit `600` → `15000`), `max-positions` `8` → `25`, `pace-per-week` `2` → `10`, `max-per-sector` `2` → `5`; `mechanism-budget-pct` is unchanged (percentage of budget, so it scales automatically: `MERGER_ARB` cap `2000` → `20000`, `QUALITY_52W_LOW` cap `1500` → `15000`); `exec-v0.6` (SP2, 2026-09) lowers `min-confidence` to `0.40`, raises `max-positions` to `8`, and adds the `MECHANISM_BUDGET` veto; `exec-v0.5` adds the buffered/capped broker stop, `atr_short`/`atrEff`, risk-based sizing with `RISK_TOO_WIDE`, and drops `NEW_HIGH` from tranche-2 eligibility in favor of gating on `entry_filled_at`; `exec-v0.4` is the sim-completion stamp (hybrid kill hard trigger, scale-out ladder, entry GTD) and also fixed a `confidence_min` drift in the seeded `rule_versions.params` audit blob (`0.6` → `0.65`, matching the `min-confidence` runtime value at the time, which was already `0.65`); `exec-v0.3` stays in the table as history — see `documentation/operations.md` for the versioning-change discipline. |
| `DRACUL_EXECUTOR_CHANDELIER_MULT` | `dracul.executor.chandelier-mult` | `3.0` | ATR multiple used by `StopRatchetService`/`MaintenancePipeline` to compute the chandelier stop level (highest price reached minus `chandelier-mult` × ATR for longs, mirrored for shorts). The active stop only ever ratchets up (in the position's favor), never down; the level is rounded to two decimals toward the safe side (down for longs, up for shorts) before the ratchet guard sees it, so the value checked, the value sent to the broker and the value written to the book are identical. Which stops a position has is read off `executor_position_leg` — one row per broker tranche, each with its own `stop_order_id` — and every leg the book still holds `OPEN` is moved to the *same* level, each addressed **by name** via `modify_bracket`'s `stopOrderId`. A protective stop is a price level on the underlying, not a per-tranche quantity, and the chandelier is a function of the high and the ATR only (it never reads the entry price), so there is no per-leg level to compute. The order id sent alongside is the leg's own `entry_order_id`, or the position's `broker_order_id` where the leg has none — Agora's `modify_bracket` resolves from the bracket, and the leg id is what selects the leg. An unnamed modify is never used to reach one of several legs: Agora's by-symbol fallback keeps only the *last* stop order on the instrument, so it would move one leg while the book recorded the new stop for both (BUG-S13). With two or more open legs where one has no `stop_order_id`, nothing is sent and it logs `ESCALATE / TRANCHE_RATCHET_UNSUPPORTED`; a position down to a **single** open leg with no id still ratchets through the bracket, where "the last stop order" and "the only stop order" are the same order. `active_stop` is written only once **every** open leg was confirmed; if an earlier leg moved and the next did not, `active_stop` keeps its **old** value (the only level true of the whole position) and it logs `ESCALATE / PARTIAL_TRANCHE_RATCHET` naming the moved and the unmoved legs — a partial is never reported as a ratchet. The next maintenance pass re-sends every leg and recovers on its own. A failed modify is classified by what the broker said: a `LEG_NOT_FOUND` rejection escalates `ESCALATE / STOP_LEG_MISSING` on the first attempt (structural, never retried) — naming the leg when the modify named one, and saying only that the broker resolved no stop leg for the bracket when it did not; any other reject code escalates `ESCALATE / STOP_MODIFY_REJECTED`; `ESCALATE / BROKER_UNAVAILABLE` now means only that the call got no verdict at all (transport failure, 5xx, timeout, or a rate limit that outlived its retries). A leg whose stop filled is closed by reconcile earlier in the same pass and is therefore never addressed, which is why the ordinary filled-leg case produces no escalation. A missing `broker_order_id` (and no leg entry id) likewise logs `ESCALATE / NO_BRACKET_ID`. Both `NO_BRACKET_ID` and `TRANCHE_RATCHET_UNSUPPORTED` cover two different situations each, so each row carries a discriminator in `inputs_snapshot` rather than only in its reasoning text: `missing` is `POSITION_BROKER_ORDER_ID` or `LEG_ENTRY_ORDER_ID_AND_POSITION_BROKER_ORDER_ID` (plus the `tranche`), and `path` is `LEG` or `COLUMN`. A rejection that is not an outage is likewise named separately everywhere it can occur — `POSITION_ALREADY_GONE` or `BROKER_REJECTED`, with the broker's own reject code in `inputs_snapshot.reject_code` — so `BROKER_UNAVAILABLE` really does mean only "no verdict". Positions that have **no** leg rows fall back to the pre-leg column routing (`stop_order_id`/`tranche2_stop_order_id`, with `stop_legs_collapsed` explaining a missing second id), unchanged. The ratchet is also skipped silently while the chandelier sits on the wrong side of the last close, or while no close is known. A transient broker failure (rate limit / HTTP 429) is retried inside the same run before escalating — see `ratchet-retry-attempts` below. The book (`executor_position.active_stop`) is written only **after** the broker confirms the modify, so it never claims a protection the broker does not hold. |
| `DRACUL_EXECUTOR_RATCHET_RETRY_ATTEMPTS` | `dracul.executor.ratchet-retry-attempts` | `3` | Total `modify_bracket` attempts `StopRatchetService` makes per leg per run before escalating `ESCALATE / BROKER_UNAVAILABLE`. Retries apply **only** to transient broker failures (rate limit / HTTP 429); a structural rejection such as a missing stop leg escalates on the first attempt as `STOP_LEG_MISSING`, because retrying it can only delay the escalation. A value of `1` restores the pre-2026-08-04 behaviour of escalating immediately. Unrelated to `max-broker-attempts`, which is a cross-run lockout counter on the *entry* path keyed on `executor_decision` rows — this one is an in-run retry of a single maintenance call and keeps no state. |
...
| `DRACUL_EXECUTOR_RATCHET_RETRY_BUDGET_MS` | `dracul.executor.ratchet-retry-budget-ms` | `20000` | Wall-clock ceiling for **all** ratchet retrying in one maintenance pass, shared across every position. It is a monotonic deadline measured from after reconcile, expiry, the indicator fetch and the hard triggers, so it bounds the ratchet phase itself, not the enclosing tool call — there is no 30 s tool timeout above it to be bounded by. It measures elapsed time in that phase, not time spent retrying. Raised 5000 -> 20000 with Agora's Saxo order-write pacer (SP3): the pacer spaces consecutive order writes by ~1.1 s, so a ten-leg ratchet pass now legitimately spends ~11 s of wall clock on FIRST attempts alone and a 5000 ms budget would be exhausted before any retry could be granted; with the pacer the first attempt is expected to succeed and the budget is never consumed. `0` disables retrying entirely. |
...
| `DRACUL_EXECUTOR_COOLDOWN_DAYS` | `dracul.executor.cooldown-days` | `3` | Days a symbol is kept in `cooldown` ("fresh setup only") after any exit (hard or soft), preventing an immediate re-entry on the same setup. The `COOLDOWN` veto is a hard block in v1 with no fresh-setup exception (see `documentation/architecture.md`). Lowered from `10` in exec-v0.8 (2026-09). |
| `DRACUL_EXECUTOR_PRICE_SANITY_PCT` | `dracul.executor.price-sanity-pct` | `0.50` | One-sided plausibility band on the favourable-extreme ratchet in `ReconcileService.updateMaintenance`. A long whose broker `marketPrice` exceeds `max(highest_price, entry_price) × (1 + pct)` — or a short that falls below `× (1 − pct)` — does **not** ratchet: `highest_price` and `mfe_r` keep their old values and one `decision_log` row (`trigger_type=MAINTENANCE`, `action=ESCALATE`, `reason_code=PRICE_IMPLAUSIBLE`) is written. Maintenance is still persisted and the position stays a survivor. The band is deliberately one-sided: a drawdown cannot corrupt an extreme that only moves in the position's favour, and escalating one would be noise. It repeats every pass while the broker keeps reporting the bad price — that is a data problem the operator must see daily. Entry-price and quantity sync are untouched (they are book=broker truths); the guard covers only the extreme that feeds the giveback stop. A **null** `marketPrice` also skips the ratchet, without escalating — it used to NPE and abort the whole reconcile pass. |
| `DRACUL_EXECUTOR_TOTAL_BUDGET` | `dracul.executor.total-budget` | `100000` | Total capital (account currency) the executor is allowed to deploy across all open positions. Divided by `tranche-count` for the per-tranche notional; the `BUDGET`/`HEAT_LIMIT` vetos and `PositionSizer` all measure against this. Raised from `10000` to `100000` in the exec-v0.7 paper capital scale-up (2026-09). |
| `DRACUL_EXECUTOR_TRANCHE_COUNT` | `dracul.executor.tranche-count` | `25` | Number of equal-sized tranches `total-budget` is divided into; each `place-entry`/`add-tranche` sizes one tranche (`total-budget / tranche-count`, FX-converted to instrument currency). Raised from `10` to `25` in the exec-v0.7 paper capital scale-up (2026-09), keeping the per-tranche notional at `4000` (was `1000`). |
| `DRACUL_EXECUTOR_HEAT_PCT` | `dracul.executor.heat-pct` | `0.15` | Fraction of `total-budget` the sum of all open positions' risk (`qty × (entry − active stop)`, account ccy) may not exceed; enforced by the `HEAT_LIMIT` veto (and mirrored in `add-tranche`'s heat check). Raised from `0.06` to `0.15` in the exec-v0.7 paper capital scale-up (2026-09), i.e. a heat limit of `15000` at the default `total-budget`. |
| `DRACUL_EXECUTOR_MAX_PER_SECTOR` | `dracul.executor.max-per-sector` | `5` | Maximum open positions per sector (case-insensitive match on the Agora company-profile lookup); enforced by the `CONCENTRATION` veto. Raised from `2` to `5` in the exec-v0.7 paper capital scale-up (2026-09). |
...
| `DRACUL_EXECUTOR_MAX_SIGNAL_AGE_DAYS` | `dracul.executor.max-signal-age-days` | `5` | Maximum signal age, in trading days since `createdAt`, before the `SIGNAL_EXPIRED` veto rejects it, and before the maintenance-pass sweeper (`PendingSignalSweeper`) retires a still-PENDING signal that was never evaluated. |
...
| `DRACUL_EXECUTOR_PACE_PER_WEEK` | `dracul.executor.pace-per-week` | `10` | Maximum new positions (tranche-1 entries) per ISO calendar week; enforced by the `PACE_LIMIT` veto. Raised from `2` to `10` in the exec-v0.7 paper capital scale-up (2026-09). |
...
| `DRACUL_OUTCOME_RECONSTRUCT_ANCHORS_ENABLED` | `dracul.outcome.reconstruct-anchors.enabled` | `true` | Master switch for `AnchorReconstructionStep`, which runs inside `OutcomeBatchJob.run()` right before the counterfactual walk. Reconstructs the two counterfactual anchor inputs (`reference_bar_date`, `reference_atr`) for signals emitted before V49 added them at emission (LLM SKIPs and swept-unevaluated signals), from today's daily OHLC history, so those signals can finally be walked. A reconstruction failure never aborts the night's counterfactuals (own try/catch in `OutcomeBatchJob.run()`). |
