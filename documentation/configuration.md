...
| `LAZARUS_MAX_CONSECUTIVE_FAILURES` | `10` | Consecutive pre-filter **failures** (an unusable answer, a symbol a chunk did not answer for, or a dead chunk call). Counted in **symbols**, not chunks: a wholly failed chunk contributes as many consecutive failures as it had symbols, and the pass stops mid-chunk on the threshold, so the number means the same thing whatever `LAZARUS_PROBE_CHUNK_SIZE` is before the source counts as down and the pass stops. One failure proves nothing — a run of them does. A symbol that is simply younger than 52 weeks is **not** a failure and does not count here (since 2026-08-06): index constituents are walked in list order, so a cluster of new listings would otherwise abort the pass against a perfectly healthy source. |
