...
| `DRACUL_EXECUTOR_MAX_BROKER_ATTEMPTS` | `dracul.executor.max-broker-attempts` | `3` | Number of BROKER_ERROR attempts before the executor stops attempting for a signal; below it a pending signal stays PENDING for retry. The counter is the signal's `executor_decision` rows with `reject_reason='BROKER_ERROR'`, and the cap covers **both** broker-write paths, not just entries: on `place-entry` the pending signal is terminally `REJECTED`, on `add-tranche` no further tranche is placed and the call answers `MAX_BROKER_ATTEMPTS` (a tranche has no signal status of its own — its source signal is long `ACCEPTED`). Both paths share the one counter, and both first try to adopt an already-live broker order via `orderByRef` before placing again. |
