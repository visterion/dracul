...
| `DRACUL_EXECUTOR_MIN_CONFIDENCE` | `dracul.executor.min-confidence` | `0.65` | `VetoService` rejects (`LOW_CONFIDENCE`) any signal whose `confidence` is below this threshold. Raised from `0.6` as part of the entry-completeness work (task 5/`VetoService`'s Java-level default); note that `application.yaml`'s own `${DRACUL_EXECUTOR_MIN_CONFIDENCE:0.6}` expression was not updated in the same change, so until that file's literal fallback is bumped to `0.65` the value actually seen at runtime (absent an explicit env var) is still `0.6` — flagged here rather than silently documented as fixed. |
