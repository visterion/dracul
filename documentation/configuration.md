...
| `DRACUL_EXECUTOR_RULE_VERSION` | `dracul.executor.rule-version` | `exec-v0.7` | The active rule-version tag (`RuleVersionProvider`) stamped onto every `decision_log` row, so a later change in prompt/thresholds is traceable in the audit trail. `RuleVersionProvider.seed()` is insert-if-absent, so bumping this default automatically seeds a new `rule_versions` row on next boot — no manual migration needed. `exec-v0.7` (2026-09) is the paper capital scale-up for learning throughput: `total-budget` `10000` → `100000`, `tranche-count` `10` → `25` (tranche `1000` → `4000`), `risk-pct` `0.01` → `0.005`, `heat-pct` `0.06` → `0.15` (heat limit `600` → `15000`), `max-positions` `8` → `25`, `pace-per-week` `2` → `10`, `max-per-sector` `2` → `5`; `mechanism-budget-pct` is unchanged (percentage of budget, so it scales automatically: `MERGER_ARB` cap `2000` → `20000`, `QUALITY_52W_LOW` cap `1500` → `15000`); `exec-v0.6` (SP2, 2026-09) lowers `min-confidence` to `0.40`, raises `max-positions` to `8`, and adds the `MECHANISM_BUDGET` veto; `exec-v0.5` adds the buffered/capped broker stop, `atr_short`/`atrEff`, risk-based sizing with `RISK_TOO_WIDE`, and drops `NEW_HIGH` from tranche-2 eligibility in favor of gating on `entry_filled_at`; `exec-v0.4` is the sim-completion stamp (hybrid kill hard trigger, scale-out ladder, entry GTD) and also fixed a `confidence_min` drift in the seeded `rule_versions.params` audit blob (`0.6` → `0.65`, matching the `min-confidence` runtime value at the time, which was already `0.65`); `exec-v0.3` stays in the table as history — see `documentation/operations.md` for the versioning-change discipline. |
