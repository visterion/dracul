...
| `LAZARUS_FETCH_TIMEOUT_SECONDS` | `600` | Value published as `webhook_timeout_seconds` on the fetch tool's definition (was a hard-coded 30 while the universe was the watchlist). It states the budget the fetch needs — 1 index call + up to `LAZARUS_UNIVERSE_MAX` pre-filter calls + `LAZARUS_FUNDAMENTALS_MAX` fundamentals calls + enrichment. **It is not enforced anywhere (verified 2026-08-04):** Vistierie declares `webhook_timeout_seconds` on the tool definition and never applies it, and the RestClient that calls the webhook uses `SimpleClientHttpRequestFactory` with an infinite read timeout. Neither the old 30 s nor this 600 s will ever cut a tool call short — do not diagnose a long-running fetch as a timeout. **Agent definitions are `insertIfAbsent` on prod — a change here needs the agent-definition reset to become visible.** |
