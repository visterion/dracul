...
| `ECHO_RECENT_NEWS_CAP` (`dracul.strigoi.echo.recent-news-cap`) | `5` (lowered from `10` on 2026-07-28) | Caps the newest-first, summary-less news **index** (`recentNews`, `{headline, source, credibility, datetime}`) carried in `fetch_recent_pead_candidates`. This is the single lever the candidate-payload byte budget depends on — raising it grows every candidate's payload back toward the ~95 kB Vistierie bridge tool-result limit that a full, summary-carrying news list once exceeded. It caps only the index: the deterministic confounder gate always scans the full, uncapped news list (summaries included), and the `fetch_candidate_news` detail tool (see `documentation/api.md`) returns the full, uncapped list with summaries for one symbol on demand. `newsCount` on each candidate reports the true, uncapped total regardless of this cap. |
