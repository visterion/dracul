...
| `DRACUL_EXECUTOR_AGORA_WRITE_TIMEOUT_MS` | `dracul.executor.agora-write-timeout-ms` | `30000` | **Read timeout only**, and only for the four broker-write tools (`place_bracket`, `flatten`, `modify_bracket`, `cancel_order`); their connect timeout stays on `agora-timeout-ms`. Agora paces Saxo order writes ~1.1 s apart and may add one clamped 429 block, so one write tool call can cover several spaced requests. **Invariant:** `(N_paced − 1) × min-interval + N_paced × request time + max-block < agora-write-timeout-ms`, with `N_paced = max(4L + 1, L + 5)` and `L` = protective legs resting on the symbol — `L = 2` (today's two-tranche position): 9 writes, 13.6 s; `L = 4` (both tranches carrying a take-profit): 17 writes, 24.0 s; both under 30 s at a 1100 ms interval. This is a **cut-off, not a bound**: Agora applies its own 10 s per request, so a badly degraded broker can exceed the cut-off and Dracul then books a non-transient `BROKER_ERROR` while Agora finishes the sequence; the next reconcile pass re-reads the broker. |
