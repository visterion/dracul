...
| Table | Purpose |
|---|---|
| `prey` | Immutable Strigoi findings; includes outcome columns filled by Voievod. Unique index `uq_prey_natural_day` (V21) on `(symbol, anomaly_type, discovered_by, user_id, discovered_at::date)` makes hunt `/complete` webhooks idempotent — a retried delivery for the same hunter/day is skipped (`ON CONFLICT DO NOTHING`) instead of duplicated, and `HuntController` emits downstream effects only for the actually-inserted subset. `run_id` (TEXT, nullable, V39) — the Vistierie run that produced this prey, taken from the `X-Vistierie-Run-Id` header and persisted by `HuntController.complete`/`PreyRepository.insertAll`; forward-only, so prey inserted before V39 stays `run_id = null`. Anchors later raw-transcript retrieval (see `documentation/vistierie-integration.md`). |
...
| `outcome_log` | (V23) One row per `decision_log` entry/reject signal, written exclusively by the nightly `OutcomeBatchJob` — the Executor itself never reads or writes this table. `kind` is `TRADE` (a closed position's realized outcome: quantity-weighted `realized_r` across partial exits, `mae_r`/`mfe_r`, `slippage_vs_limit`, whipsaw flags `reentry_within_10d`/`roundtrip_under_5d`) or `COUNTERFACTUAL` (a rejected signal's "what would have happened", via `HypotheticalREngine`: `hypothetical` JSONB with `r_after_20d`/`r_after_60d`/`would_have_stopped_out`/`skipped_reason`/`entry_source`/`entry_price` (SP8), plus `hunter_label` — the triple-barrier +1R-before-1R label used for hunter-confidence calibration). Unique on `log_id_ref` (the source `decision_log.log_id`), upserted idempotently (`ON CONFLICT ... DO UPDATE`) so re-runs refine rather than duplicate a row. See "Outcome batch job" below |
...
- **Completion needs the label horizon, not a flat 60 bars (SP12).** A
  non-skipped row only flips `complete=true` once
  `bars.size() >= max(60, horizonTradingDays)` (3m = 64, 6m = 129,
  12m = 257 trading bars) — flipping it at a flat 60 would freeze
  `hunter_label = NULL` forever on any row whose horizon needs more bars
  than that, since `HypotheticalREngine.tripleBarrierLabel` only returns a
  definite `false` ("no barrier touch") once the horizon's bar count is
  reached and `null` before it. Applies identically to both walks
  (REJECT rows here and the anchored `LLM_SKIP`/`SIGNAL_EXPIRED_UNEVALUATED`
  walk, `processSignalAnchored`).
- **`executor_signal.reference_source` (V50) and retroactive anchor
  reconstruction (SP12).** `LLM_SKIP`/`SIGNAL_EXPIRED_UNEVALUATED`
  counterfactuals need `executor_signal.reference_bar_date`/`reference_atr`,
  which `PreySignalEmitter` only started persisting at V49; rows written
  before that had neither. `reference_source` (`VARCHAR(20)`, CHECK-constrained
  to `emission`/`reconstructed`/`unreconstructable`/`manual`, nullable —
  `NULL` means no anchors and no reconstruction attempted yet) records where
  a row's anchors came from: `emission` written by `PreySignalEmitter` at
  signal time; `reconstructed` written by the new `AnchorReconstructionStep`,
  a nightly step inside `OutcomeBatchJob.run()` that runs right before the
  counterfactual walk (own try/catch — a reconstruction failure never blocks
  the walk) and backfills `reference_bar_date`/`reference_atr` for eligible
  pre-V49 rows from daily OHLC history, mirroring Agora's completed-bar rule
  and ATR calculation; `unreconstructable` marks a permanent failure (not
  retried automatically — an operator resets it to `NULL` via SQL, see
  operations.md); `manual` is the V50 migration's classification for the one
  pre-existing row whose anchors are dated after its emission (hand-written,
...
| Parsed term sheet | `distribution_ratio` (regex-parsed by `SpinTermsParser`), `record_date`, `distribution_date` (**not** regex-parsed — always written by the agent's evidence-verified reading of `term_sheet_text`, never by the parser, which returns `null` for both), `term_sheet_available`, `term_sheet_text` (raw EX-99.1 Information Statement prose, read with `extract_mode=LEADING`, kept for the LLM to read the spin thesis and quote as evidence), `parent_symbol` (best-effort parent ticker for the size-ratio), `terms_checked_at` (V44 — last time term capture was attempted for this row; throttles re-capture to once per 7 days while both dates are still null, so a row whose exhibit never resolves is not re-fetched on every hunt) |
| Lifecycle | `status` (TEXT, validated by the Java `SpinStatus` enum — **not** a DB enum, same convention as `executor_signal.status`), plus audit timestamps `discovered_at`, `last_checked_at`, `distributed_at`, `settled_at`, `abandoned_at`. These are OBSERVATION timestamps — when Dracul's reconciler noticed the transition — not market-event dates; `distributed_at` in particular can lag the real distribution by weeks to months (e.g. a backfill run that moves several long-completed spin-offs to `DISTRIBUTED` in one pass). The LLM payload's `anchorSource` (`DISTRIBUTION_DATE` \| `RECORD_DATE` \| `DETECTED`, see `documentation/strigoi.md`) names which of `distribution_date`, `record_date`, or this `distributed_at` fallback actually anchors `daysSinceDistribution` (`SpinLifecycleReconciler.promotionAnchorDate`); `distributionDateConfirmed` is `true` whenever `anchorSource != DETECTED` |
