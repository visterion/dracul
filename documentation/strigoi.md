...
**Mega-cap exemption from the cheapness gate (2026-08-09).** The cheapness gate
(`pbAnnual ≤ dracul.strigoi.lazarus.max-price-to-book` OR `price/fcfPerShare ≤
max-p-fcf`) encodes Piotroski's 2000 origin universe (the highest book-to-market
quintile) and structurally excludes very large companies, which rarely trade
below 2x book value: measured 2026-08-09, only 17 of 122 S&P 500 names above
100 Bn USD cleared it, and those were banks, oil and telecom — the cohort this
hunter surfaced in June (MSFT, ADBE, CRM, NVDA, META) has been excluded since
the gate landed. `LazarusScreener.screen` now accepts a `megaCapUsdMillions`
threshold (config `dracul.strigoi.lazarus.mega-cap-usd-millions`, env
`LAZARUS_MEGA_CAP_USD_MILLIONS`, default `100000`) and adds a third OR-branch,
`megaCap`, evaluated **only inside the cheapness check** — the solvency gate
(`roaOk || fcfOk`), the leverage cap (`maxDebtEquity`) and the unit-plausibility
guard above are unchanged and run first. `megaCap` is true only when
`marketCap() != null`, `marketCap() >= megaCapUsdMillions`, the threshold is
`> 0` (`0` disables the exemption entirely — a bare `>=` against a `0`
threshold would otherwise enable it for every candidate), and the reporting
currency is `null` or `"USD"`. The screener runs **before** enrichment, so it
sees `BasicFinancials.marketCap()` raw, in millions of the reporting currency,
not the FX-normalised `marketCapUsdMillions` the enriched wire shape carries —
a non-USD name (e.g. 0941.HK, ~226 Bn USD reported in CNY) is deliberately
**not** exempted in this slice: comparing a raw non-USD figure against a
USD threshold would be right by accident, and adding an FX dependency to this
pure, I/O-free class was judged not worth it for one gate. No new counter: the
exemption lives inside the existing cheapness check, not a separate
loss/degradation channel.
