...
class AnchorReconstructorTest {
    ...
    @Test void duplicateSameDateRowsLastWinsAndUnsortedInputIsSorted() {
        ...
        // a same-date snapshot row appended after the real bar, with a wild range
        bars.add(new OhlcBar(last.date(), last.open(), bd("500"), bd("1"), last.close(), 5));
        // unsorted input without changing the relative order of the two same-date rows
        java.util.Collections.swap(bars, 0, 5);
        var res = r.reconstruct("TESTCO", Instant.parse("2026-09-25T21:00:00Z"), bars, LocalDate.parse("2026-07-01"));
        // last-wins keeps the wild row for that date -> ATR must include its TR (500-100=400)
        var a = (AnchorReconstructor.Anchor) res;
        assertThat(a.barDate()).isEqualTo(last.date());
        assertThat(a.atr()).isGreaterThan(bd("12.0000"));
    }
    ...
    @Test void malformedBarInsideWindowFailsOutsideDoesNot() {
        ...
        OhlcBar old = bars.get(2);   // outside the last 23
        bars.set(2, new OhlcBar(old.date(), old.open(), BigDecimal.ZERO, old.low(), old.close(), 1));
        assertThat(r.reconstruct("TESTCO", Instant.parse("2026-09-11T21:00:00Z"), bars, LocalDate.parse("2026-07-01")))
                .isInstanceOf(AnchorReconstructor.Anchor.class);
        OhlcBar inside = bars.get(20);
        bars.set(20, new OhlcBar(inside.date(), inside.open(), inside.high(), inside.low(), BigDecimal.ZERO, 1));
        assertThat(r.reconstruct("TESTCO", Instant.parse("2026-09-11T21:00:00Z"), bars, LocalDate.parse("2026-07-01")))
