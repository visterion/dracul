...
class DaywalkerEventEngineTest {
    ...
    private DaywalkerEventEngine engine(HeldPositionService hp, AgoraIntraday in,
                                        AgoraCompanyData cd, AgoraFilings fi,
                                        DaywalkerAlertRepository al) {
        return new DaywalkerEventEngine(hp, in, cd, fi, al, 0.03, 3.0, 3600, "depot-1", "u1@x.com");
    }
    ...
    void emitsPriceSpikeAndAppliesCooldown() {
        ...
        List<TriggerEvent> events = engine(hp, in, cd, fi, al).detect(null, now);
        ...
        assertThat(engine(hp, in, cd, fi, al).detect(null, now)).isEmpty();
    }
    ...
    void dedupsSameSymbolAndFetchesMarketDataOnce() {
        ...
        var events = engine(hp, in, cd, fi, al).detect(null, Instant.parse("2026-06-03T18:00:00Z"));
    ...
    void emptyDepotReturnsNoEvents() {
        var hp = mock(HeldPositionService.class);
        when(hp.openPositions("depot-1")).thenReturn(List.of());
        var engine = engine(hp, mock(AgoraIntraday.class), mock(AgoraCompanyData.class),
    ...
    void multiplePositionsEmitPerSymbolEventsWithIndependentCooldown() {
        ...
        var now = Instant.parse("2026-06-03T18:00:00Z");
        var events = engine(hp, in, cd, fi, al).detect(null, now);
        ...
        var events2 = engine(hp, in, cd, fi, al).detect(null, now);
