...
class Tranche2DetectorTest {
    ...
    void rConfirmed_atExactlyPlusOneR_eligible() {
        ...
        Tranche2Detector.Tranche2Status status = detector.detect(p, new BigDecimal("110"), List.of());
    ...
    void belowEntry_neverEligible_evenWithReinforcingSignal() {
        ExecutorPosition p = position(1, "BUY", new BigDecimal("100"), new BigDecimal("90"),
                null, "ACME", "OPEN");
        List<ExecutorSignal> pendings = List.of(
                signal("ACME", "BUY", "PEAD"),
                signal("ACME", "BUY", "SPIN_OFF"));

        Tranche2Detector.Tranche2Status status = detector.detect(p, new BigDecimal("99"), pendings);
    ...
    void sellMirror_rConfirmedAndNeverAboveEntry() {
        ...
        Tranche2Detector.Tranche2Status confirmed = detector.detect(p, new BigDecimal("90"), List.of());
    ...
    void notOpenOrNullPrice_notEligible() {
        ExecutorPosition closed = position(1, "BUY", new BigDecimal("100"), new BigDecimal("90"),
                null, "ACME", "CLOSED");
        assertThat(detector.detect(closed, new BigDecimal("110"), List.of()).eligible()).isFalse();

        ExecutorPosition open = position(1, "BUY", new BigDecimal("100"), new BigDecimal("90"),
                null, "ACME", "OPEN");
        assertThat(detector.detect(open, null, List.of()).eligible()).isFalse();
    }
