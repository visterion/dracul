...
public class VoievodRegistrar {

    private static final Logger log = LoggerFactory.getLogger(VoievodRegistrar.class);
    private static final String AGENT_NAME = "voievod";
    ...
    public void register() {
        try {
            ...
            if (existing.isEmpty()) {
                ...
                log.info("voievod agent registered with Vistierie");
                return;
            }
            if (!matches(existing.get(), desired)) {
                ...
                log.info("voievod agent updated in Vistierie");
            } else {
                log.info("voievod agent registration is up-to-date");
            }
        } catch (Exception e) {
            log.warn("voievod registration failed: {}", e.getMessage());
        }
    }
    ...
    private CreateAgentRequest buildRequest() {
        ...
        try {
            inputSchema = mapper.readTree("{\"type\":\"object\",\"properties\":{}}");
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        var tool = new ToolDef(
                "fetch_consensus_clusters",
                "Returns symbols flagged by two or more different Strigoi, with the contributing prey.",
                inputSchema,
                null, null,
                publicUrl + "/api/voievod/tools/fetch-candidates",
                30
        );
        return new CreateAgentRequest(
                ...
                publicUrl + "/api/voievod/complete",
                webhookToken
        );
    }
