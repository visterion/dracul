...
public class StrigoiMergerRegistrar {

    private static final Logger log = LoggerFactory.getLogger(StrigoiMergerRegistrar.class);
    private static final String AGENT_NAME = "strigoi-merger";
    ...
    public StrigoiMergerRegistrar(
            VistierieClient vistierie,
            ObjectMapper mapper,
            @Value("${dracul.public-url}") String publicUrl,
            @Value("${dracul.strigoi.merger.webhook-token}") String webhookToken,
    ...
    public void register() {
        try {
            ...
            if (existing.isEmpty()) {
                ...
                log.info("strigoi-merger agent registered with Vistierie");
                return;
            }
            if (!matches(existing.get(), desired)) {
                ...
                log.info("strigoi-merger agent updated in Vistierie");
            } else {
                log.info("strigoi-merger agent registration is up-to-date");
            }
        } catch (Exception e) {
            log.warn("strigoi-merger registration failed: {}", e.getMessage());
        }
    }

    @EventListener(LanguageChangedEvent.class)
    public void onLanguageChanged(LanguageChangedEvent event) {
        log.info("language changed to {}; re-registering strigoi-merger", event.language());
        register();
    }

    private CreateAgentRequest buildRequest() {
        var prompt = LanguageDirective.append(
                readClasspath("prompts/strigoi-merger.md"), settings.getLanguage());
        JsonNode schema;
        try {
            schema = mapper.readTree(readClasspath("schemas/prey-list-merger.json"));
        } catch (Exception e) {
        ...
        return new CreateAgentRequest(
                ...
                publicUrl + "/api/strigoi-merger/complete",
                webhookToken
        );
    }
