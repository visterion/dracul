...
public class StrigoiEchoRegistrar {

    private static final Logger log = LoggerFactory.getLogger(StrigoiEchoRegistrar.class);
    private static final String AGENT_NAME = "strigoi-echo";
    ...
    public void register() {
        try {
            ...
            if (existing.isEmpty()) {
                ...
                log.info("strigoi-echo agent registered with Vistierie");
                return;
            }
            if (!matches(existing.get(), desired)) {
                ...
                log.info("strigoi-echo agent updated in Vistierie");
            } else {
                log.info("strigoi-echo agent registration is up-to-date");
            }
        } catch (Exception e) {
            log.warn("strigoi-echo registration failed: {}", e.getMessage());
        }
    }
    ...
    private CreateAgentRequest buildRequest() {
        ...
        var tool = new ToolDef(
                "fetch_recent_pead_candidates",
                "Returns positive-surprise PEAD candidates detected in the last N days.",
                inputSchema,
                null, null,
                publicUrl + "/api/strigoi-echo/tools/fetch-candidates",
                30
        );
        return new CreateAgentRequest(
                ...
                publicUrl + "/api/strigoi-echo/complete",
                webhookToken
        );
    }
