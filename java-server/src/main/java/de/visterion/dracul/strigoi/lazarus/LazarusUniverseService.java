 ...
 *
 * <p><b>Three bounded ways to lose symbols, all counted.</b> None of them may look like a quiet
 * market:
 * <ul>
 *   <li><b>{@code probeFailed}</b> — Agora answered for a symbol but the range is unusable (no
 *       history, freshly listed), or the single call failed. Reported as {@code partial}.</li>
 ...
 *   <li><b>{@code sourceDown}</b> — a RUN of consecutive failures. Agora reports "no history for
 *       this symbol" through the same envelope as "Agora is unreachable", so one failure proves
 *       nothing; a run of them does, and stopping there saves hundreds of dead remote calls.</li>
 * </ul>
 */
@Component
public class LazarusUniverseService {
     ...
     * @param probeFailed probed symbols that yielded no usable range
    ...
    public record Scan(List<PreScreened> shortlist, int considered, int screened,
                       int probeFailed, int unscreened, boolean sourceDown) {}
    ...
    public Scan preScreen(List<IndexConstituent> universe, double margin, long budgetMs,
        ...
        if (size == 0) return new Scan(List.of(), 0, 0, 0, 0, false);
        ...
        for (int i = 0; i < size; i++) {
            ...
            screened++;
            try {
                PriceRange r = priceRange.range52w(c.symbol());
                if (r == null) {
                    probeFailed++;
                    consecutiveFailures++;
                } else {
        ...
        return new Scan(List.copyOf(shortlist), size, screened, probeFailed, size - screened, sourceDown);
    }
}
