...
public class StrigoiInsiderWebhookController {

    private static final Logger log = LoggerFactory.getLogger(StrigoiInsiderWebhookController.class);
    ...
    public ResponseEntity<Map<String, Object>> fetchClusters(
        ...
        var filings = edgar.recentFilings(from, to);
        var clusters = screener.cluster(filings);
        return ResponseEntity.ok(Map.of("output", Map.of("clusters", clusters)));
    }
    ...
    public ResponseEntity<Void> complete(
        ...
        if (!preyNode.isArray() || preyNode.isEmpty()) {
            log.info("strigoi-insider run {} produced no prey", runId);
            return ResponseEntity.noContent().build();
        }
        ...
        for (JsonNode p : preyNode) {
            ...
            prey.add(new Prey(
                    ...
                    p.path("anomalyType").asText("INSIDER_CLUSTER"),
        ...
        log.info("strigoi-insider run {} persisted {} prey", runId, prey.size());
        return ResponseEntity.noContent().build();
    }
}
