...
- `daysSinceDistribution` — days since the spin-co's EFFECTIVE distribution
  date. That date is the term-sheet `distributionDate` when known; otherwise it
  falls back to the date Dracul first *detected* the spin-co trading, which can
  lag the real distribution by weeks to months (this happens for candidates
  whose term sheet never carried a distribution date — a common gap). **Check
  `distributionDateConfirmed` before treating a small `daysSinceDistribution`
  as "the window just opened":** `true` means it is measured from the real
  event and freshness is trustworthy; `false` means it is only Dracul's
...
- **High confidence** — DISTRIBUTED, in an open window (small
  `daysSinceDistribution` with `distributionDateConfirmed=true`, or otherwise
  corroborated by the term sheet / filing recency — never a small
  `daysSinceDistribution` alone when `distributionDateConfirmed=false`), with
  size evidence pointing to mandate selling
...
`daysSinceDistribution`, and `sizeRatio`. When `distributionDateConfirmed=false`,
phrase any `daysSinceDistribution`-based criterion off today's calendar date instead
(e.g. "dead if no re-rating by <today + N days>"), since the field is not anchored to
a known event date in that case. The payload carries no live price (only the
